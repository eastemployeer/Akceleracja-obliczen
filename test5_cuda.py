...
@jit
def countMSE_CUDA(image_in, image_out):
    sum = 0
    for x in range(image_in.shape[0]):
        for y in range(image_in.shape[1]):
            for z in range(3):
...
print("Counting MSE without GPU:", timer()-start)

start = timer()
MSE_result = countMSE_CUDA(image_rgb, result_image)
print("Błąd: {}".format(MSE_result))
print("Counting MSE with GPU:", timer()-start)